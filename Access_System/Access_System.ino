...
void setup() {
  ...
  pinMode(pinMinX, INPUT_PULLUP);
  pinMode(pinMaxX, INPUT_PULLUP);
}
...
void loop() {
  ...
  if(STATE_P==1){
   ...
   if(STATE_P==0){  //si le signal est a l'etat haut
    if(digitalRead(IR_output)==HIGH){
      Serial.println("0x3");
    }
   }
...
void detection_IR(){
  if(digitalRead(IR_output)==HIGH){  //si le signal est à l'état haut
   if(STATE_P==1){
    Serial.print("0x4");
    //TROUVER MOYEN TEMPO L'ENVOI AU SERV pour pas flood le serv
    }
   if(STATE_P==0){
    Serial.print("0x3");
    //TROUVER MOYEN TEMPO L'ENVOI AU SERV pour pas flood le serv
